...
Set-Location $ROOT
Write-Host ""
Write-Host "COBO LitVM Testnet Lab v1.4 running at:" -ForegroundColor Green
Write-Host "http://127.0.0.1:$PORT/index.html" -ForegroundColor Cyan
Write-Host "DO NOT CLOSE THIS WINDOW. Press CTRL + C to stop." -ForegroundColor Yellow
$connections = Get-NetTCPConnection -LocalPort $PORT -ErrorAction SilentlyContinue
if ($connections) {
  $pids = $connections | Select-Object -ExpandProperty OwningProcess -Unique
  foreach ($pidItem in $pids) { try { Stop-Process -Id $pidItem -Force -ErrorAction SilentlyContinue } catch {} }
}
...
while ($true) {
  ...
  try {
    ...
    if (!(Test-Path $filePath)) {
      ...
      $header = "HTTP/1.1 200 OK`r`nContent-Type: $(Get-ContentType $filePath)`r`nContent-Length: $($body.Length)`r`nConnection: close`r`n`r`n"
    }
